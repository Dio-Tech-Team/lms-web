<template>
  ...
  >
    <div class="bg-white rounded-2xl border border-sky-100 p-7 w-full max-w-md">
      <h2 class="font-serif text-xl font-semibold text-navy-deep mb-1">Mark as Resigned</h2>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="e.g. Resigned to pursue opportunity abroad"
          ></textarea>
        </div>
        ...
        <div class="flex gap-2 pt-2">
          ...
          >
            {{ submitting ? 'Saving...' : 'Confirm Resignation' }}
          </button>
        </div>
      </form>
    </div>
...
async function handleSubmit() {
  ...
  error.value = ''
  try {
    await api.post(`/employees/${props.employeeId}/resign`, {
    ...
    error.value = err.response?.data?.message || 'Failed to process resignation.'
  } finally {
